...
export default function AuthScreen({ onAuthSuccess }: AuthScreenProps) {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Login flow warning (handled):", err.message || err);
      let friendlyMessage = "Failed to sign in. Please check your credentials.";
      const msg = err?.message || "";
      const code = err?.code || "";
      if (code === "auth/invalid-credential" || msg.includes("invalid-credential") || msg.includes("auth/invalid-credential") || msg.includes("auth/wrong-password") || msg.includes("wrong-password")) {
        friendlyMessage = "Incorrect email or password. Please try again.";
      } else if (code === "auth/user-not-found" || msg.includes("user-not-found")) {
        friendlyMessage = "No account found with this email. Please create an account.";
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Signup flow warning (handled):", err.message || err);
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Password reset flow warning (handled):", err.message || err);
      let friendlyError = "Failed to send reset email. Verify your address is correct.";
      const msg = err?.message || "";
      const code = err?.code || "";
      if (code === "auth/user-not-found" || msg.includes("user-not-found") || msg.includes("auth/user-not-found")) {
        friendlyError = "No account found with this email address. Please check spelling or create an account.";
      } else if (code === "auth/invalid-email" || msg.includes("invalid-email")) {
